...
class all_callbacks:
    def __init__(
        ...
        self.modelbestcheck = ModelCheckpoint(filepath =
            os.path.join(outputDir, "KERAS_check_best_model.keras"), monitor='val_loss', verbose=1, save_best_only=True
        )

        self.modelbestcheckweights = ModelCheckpoint(
            filepath = os.path.join(outputDir, "KERAS_check_best_model.weights.h5"),
            monitor='val_loss',
            verbose=1,
            save_best_only=True,
        ...
        self.modelcheckperiod = ModelCheckpoint(filepath = os.path.join(outputDir, "KERAS_check_model_epoch{epoch:02d}.keras"), verbose=1, save_freq = 'epoch')

        self.modelcheck = ModelCheckpoint(filepath = os.path.join(outputDir, "KERAS_check_model_last.keras"), verbose=1)

        self.modelcheckweights = ModelCheckpoint(
            filepath = os.path.join(outputDir, "KERAS_check_model_last.weights.h5"), verbose=1, save_weights_only=True
        )

        self.tb = TensorBoard(log_dir=os.path.join(outputDir,'logs'))
