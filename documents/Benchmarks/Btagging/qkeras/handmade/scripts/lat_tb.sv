...
module lat_tb;
    ...
    initial begin
        if (write_file) begin
            fd = $fopen("../reports/hand_lat.csv", "a");  // "w" = write mode, "a" = append
            if (fd == 0) begin
        ...
        input_ready<=0;
        @(posedge ready)
        ii_count=iteration_count;
        @(posedge output_ready);
        $fwrite(fd, "%0d, %0d, %0d\n",  width, ii_count, iteration_count);
        if (write_file) begin
            $fclose(fd);
        end
        $stop;
