...
def dump_weights(model):
    for layer in model.layers:
        np.savetxt(f"./weights/weights_{layer.name}_weights_biases_pkgs/{layer.name}_weights.txt", layer.get_weights()[0])
        np.savetxt(f"./weights/weights_{layer.name}_weights_biases_pkgs/{layer.name}_biases.txt", layer.get_weights()[1])

def gen_weight_txt(accuracy): #, weights_file, biases_file
    """
    ...
    for ind in range(len(layers)):
        ...
        weights_file = f"./weights/weights_dense_{ind}_weights_biases_pkgs/dense_{ind}_weights.txt"
        biases_file = f"./weights/weights_dense_{ind}_weights_biases_pkgs/dense_{ind}_biases.txt"
        ...
        # Output file name
        filename = f"./weights/weights_dense_{ind}_weights_biases_pkgs/dense_{ind}_pkg_{accuracy[0]}_{accuracy[1]}.sv"
        #f"./weights/weights_{name}_weights_biases_pkgs/{name}_pkg_{accuracy[0]}_{accuracy[1]}_{i}.sv"
