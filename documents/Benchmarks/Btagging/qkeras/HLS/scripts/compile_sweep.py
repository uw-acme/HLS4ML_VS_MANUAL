...
import yaml
path.insert(1,"/home/quin/HLS4ML_VS_MANUAL/documents/Benchmarks/Btagging/qkeras/")
from helper_functions import *
# import subprocess
from tensorflow.keras.models import load_model # type: ignore

model = load_model('lstm_test2/lstm_weights.h5', compile=False)
...
test = np.resize(x_test_lstm_inport, (x_test_lstm_inport.shape[0], 15, 6))
...
def HLS4ML_gen(acc : tuple[int, int]):
    ...
    fullsweepname = f"/home/quin/HLS4ML_VS_MANUAL/documents/Benchmarks/Btagging/qkeras/models/lstm_floating_point/{sweepname}"
...
def test_accuracy(model, acc):
    patt = r"[0-9]{1,2}"
    gen_test(acc, test)
    os.system(f'sed -i -E "s/NINT = {patt}/NINT = {acc[1]}/g; s/WIDTH = {patt}/WIDTH = {acc[0]}/g;" hls_tb.sv')
