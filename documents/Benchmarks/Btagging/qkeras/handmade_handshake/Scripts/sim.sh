...
# Dense layer
xvlog --sv LSTM.sv ${1//' '/ -d } Btagging.sv ${1//' '/ -d } Btagging_top.sv ${1//' '/ -d } adderTree.sv ${1//' '/ -d } adderTree_p4.sv ${1//' '/ -d } denseLayer.sv ${1//' '/ -d } relu.sv ${1//' '/ -d } shift_add.sv ${1//' '/ -d } sigmoid.sv ${1//' '/ -d } tanh.sv ${1//' '/ -d }
...
# Elaborate design
xelab Btagging_top_tb ${1//' '/ -d } ${2//' '/ -generic_top } -s Btagging_sim --debug typical 

# Run simulation to completion (headless, no GUI waves)
xsim Btagging_sim -runall
