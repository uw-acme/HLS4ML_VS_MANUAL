...
module Btagging_top #( parameter
    ...
    logic input_ready_Btagging, output_ready_Btagging, Btagging_ready;
    assign output_ready = output_ready_Btagging;
    logic [$clog2(TIMESTEPS):0] step=0;
    assign ready = Btagging_ready;
    always_ff @(posedge shiftClk) begin
        input_ready_Btagging<=0;
        if (input_ready) begin
            input_v[step]<=input_step;
            step<=(step!=(TIMESTEPS-1) ? step+1 : 0);
            input_ready_Btagging<=step==(TIMESTEPS-1);
        end
    end
    Btagging #(.WIDTH(WIDTH), .NINT(NINT)) Btag (.clk, .reset, .ready(Btagging_ready), .input_ready(input_ready_Btagging), .output_ready(output_ready_Btagging), .input_v, .output_data);
endmodule

`ifndef SYNTHESIS
`define STRINGIFY(x) `"x`"
module Btagging_top_tb;
    logic clk, shiftClk;
    logic reset;
    logic input_ready;
    logic output_ready;
    ...
    Btagging_top #(.WIDTH(WIDTH), .NINT(NINT)) dut (.*);
    initial begin
        clk=0;
        forever #1 clk<=~clk;
    end
