 ...
 #(
  ...
  (
   ...
   always_ff @(posedge clk) begin
	...
	always_ff @(posedge clk) begin
		if (reset) begin
			...
			if (col == inputWidth - 1) begin
				...
				// row <= row + 1;
				// evenRow <= ~evenRow;
				if (row == inputWidth -1) begin	// start of a new image
					row <= 0;
					evenRow <= 1'b1;
				end else begin
					row <= row + 1;
	...
	// Signals when at image boundary (need to reset FIFO)
	logic imageBoundary;
	assign imageBoundary = continueCalculations && (row == inputWidth-1) && (col == inputWidth -1);
