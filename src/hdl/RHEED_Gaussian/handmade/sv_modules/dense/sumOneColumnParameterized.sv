...
module sumOneColumnParameterized #(parameter WIDTH = 16, NFRAC = 10, INPUT_SIZE = 10, OUTPUT_SIZE = 15, ITER=0) (clk, data, bias, out);
    input logic clk;
    ...
    generate
        for (y=0; y < INPUT_SIZE; y++) begin: shift_adds
            // uses shift add
            shift_add_with_mult #(dense_test_data::dlWeights[(y*OUTPUT_SIZE+(ITER))], 3, WIDTH,NFRAC) 
