...
// ENABLED VERSION
`timescale 1ns / 1ps
module conv2dFIFO_enabled #(parameter filtDimension = 3, parameter bitWidth = 8, parameter inputWidth = 8) 
(clock, inputData, en, outputData);
	...
	// if enabled, shift all bits to the left by one and shift in the inputData
	always_ff @(posedge clock) begin
        if (en) 
			if (inputWidth-filtDimension == 1) begin // if only 1 bit long
				sr_data <= inputData;
			end else begin
...
module conv2dFIFO_enabled_testbench();
	logic clock, en; //reset;
	logic [2:0] inputData;
	logic [2:0] outputData;
	
	conv2dFIFO_enabled #(3,3,8) dut (.clock(clock), .inputData(inputData), .outputData(outputData));
