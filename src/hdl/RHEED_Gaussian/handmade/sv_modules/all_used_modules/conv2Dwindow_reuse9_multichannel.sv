/*
	This module creates a multichannel 2D convolution window for a no-padding implementation.
	Each input pixel is considered to have depth = inputChannels 
	(every value that shares the same [row][col] is assumed to stream in simultaneously)
*/
...
`timescale 1ns / 1ps
module conv2Dwindow_reuse9_multichannel
#(parameter inputChannels = 1, parameter filtDimension = 3, parameter bitWidth = 16, parameter inputWidth = 8)
(clock, reset, inputPixel, shiftEnable, currConvMatrix, positionValid, nextPositionValid);
	input logic clock, reset;
	input logic signed [bitWidth-1:0] inputPixel [0:inputChannels-1];
	input logic shiftEnable; 

	output logic signed [bitWidth-1:0] currConvMatrix [0:inputChannels-1][filtDimension-1:0][filtDimension-1:0];
	...
	logic signed [bitWidth-1:0] outputFifo [0:inputChannels-1][filtDimension-2:0]; // collection of buffers (inputChannels x filtDimension-1 many buffers)
				...
				for(col=0; col<filtDimension; col++) begin
					...
					if(shiftEnable) begin 
							...
							if(col==filtDimension-1 && row != filtDimension -1) begin	// right edge of matrix, not bottom row
								currConvMatrix[ch][row][col] <= outputFifo[ch][row];
							end else if(row==filtDimension-1 && col==filtDimension-1) begin	// bottom right pixel
								// bring in new value
								currConvMatrix[ch][row][col] <= inputPixel[ch];	
							end else begin
								// shift normally
								currConvMatrix[ch][row][col] <= currConvMatrix[ch][row][col+1];
							end
					end
				end
			end
	...
	genvar i, j;	// i represents the row
	generate 
		for(j = 0; j<inputChannels; j++) begin: channelBuffers
			for(i = 0; i<filtDimension-1; i++) begin: buffers
				...
				(.inputData(currConvMatrix[j][i+1][0]), .clock(clock), .en(shiftEnable), .outputData(outputFifo[j][i]));
			end
		end
	endgenerate
...
// 	conv2Dwindow_reuse9_multichannel #(3,16,8) dut(.*);
