...
(clk, reset, inputPixel, biases, convWeights, inputValid, inputReady, outputReady, outputPixel, outputValid);
	...
	conv2Dwindow_reuse9_multichannel #(inputChannels, filtDimension, bitWidth, inputWidth) currMatrix 
	...
	generate
	   for(i=0; i<biasWidth; i++) begin: eachSumNine // biasWidth = number of filters
		   conv2Dsum_reuse9_multichannel #(inputChannels, filtDimension, bitWidth, NFRAC, i, biasWidth) conv2Dsum
	       (.clock(clk), .reset(reset), .start(startSum), .currMatrix(currConvMatrix), .counter(counter), .bias(biases[i]), .convWeights(convWeights), .sum(sum[i]), .outputValid(sumOutValid[i]));
	   end
	endgenerate 
	...
	always_ff @(posedge clk) begin
		...
		else if (allSumsValid && outputValid && !outputReady) begin
			sumWaiting <= 1;
		end
		...
		case (ps) 
			...
			STALL: begin
				...
				if (!outputValid || outputReady) begin  // otherwise stall: current pixel is not yet consumed
					// either no output pending to protect, or downstream module consumes it now, so
					// safe to continue
						if (inputValid) begin
							inputReady = 1;
							shiftEnable = 1;
						...
						// if inputValid isn't true yet, stay in STALL (ns=ps)
					end
				// end
			end
		endcase
