...
`timescale 1ns / 1ps
module conv2d_reuse9_with_relu_multichannel_tb_wrapper();
    parameter inputChannels = 8, filtDimension = 3, bitWidth = 16, inputWidth = 48, biasWidth = 6, NFRAC = 10;
    ...
    logic signed [bitWidth-1:0] inputMatrix [0:inputChannels-1][0:inputWidth**2-1];
    logic signed [bitWidth-1:0] outputMatrix_sc [TOTAL_PIXELS*biasWidth-1:0];

    assign inputMatrix = conv2d_reuse9_multichannel_test_data::inputMatrix;
    assign biases = conv2d_reuse9_multichannel_test_data::convBiases;


	// dut
	conv2d_output_with_relu_reuse9_multichannel #(inputChannels, filtDimension, bitWidth, inputWidth, biasWidth, NFRAC) dut (.*);

    // true output generation
    conv2d_reuse9_multichannel_single_cycle #(inputChannels, inputWidth, filtDimension, bitWidth, biasWidth, NFRAC) true_outputs (.*);
    ...
    initial begin
        ...
        for (int ch = 0; ch < inputChannels; ch++) begin
            inputPixel[ch] <= inputMatrix[ch][j]; // first pixel, all channels
        end
        ...
        while (j<inputWidth**2) begin
			...
			if (inputReady && inputValid) begin // pixel  accepted
                ...
                if (j < inputWidth**2) begin
                    for (int ch = 0; ch < inputChannels; ch++) begin
                        inputPixel[ch] <= inputMatrix[ch][j]; // next pixel, all channels
                    end
                end
			end
        end
    ...
    always_ff @(posedge clk) begin
        if (reset) begin
            ...
            if (outputValid && outputReady) begin
                $display("time: %0t, validOutputIdx: %0d, outputValid: %b,  outputReady: %b, ps: %s, counter: %0d", 
                         $time, validOutputIdx, outputValid, outputReady, dut.ps, dut.counter);
