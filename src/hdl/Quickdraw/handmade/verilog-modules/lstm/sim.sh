...
# cd ../verilog-modules
rm -rf xsim.dir/ quickdraw_lstm_sim.jou quickdraw_lstm_sim.log sim.log
...
# Dense layer
xvlog --sv LSTM.sv ${1//' '/ -d } quickdraw_lstm.sv ${1//' '/ -d } adderTree.sv ${1//' '/ -d } adderTree_p4.sv ${1//' '/ -d } denseLayer.sv ${1//' '/ -d } relu.sv ${1//' '/ -d } softmaxLayerNeg.sv ${1//' '/ -d } shift_add.sv ${1//' '/ -d } sigmoid.sv ${1//' '/ -d } tanh.sv ${1//' '/ -d }
...
# Elaborate design
xelab Quickdraw_LSTM_tb ${1//' '/ -d } ${2//' '/ -generic_top } -s quickdraw_lstm_sim --debug typical 

# Run simulation to completion (headless, no GUI waves)
xsim quickdraw_lstm_sim -runall
