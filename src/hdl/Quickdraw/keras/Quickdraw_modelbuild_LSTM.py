...
def learningCurve(history):
    ...
    plt.savefig('LSTM_learning_curve.pdf')
    plt.show()
    plt.close()
...
try:
    ...
    print(auc_score)
    lstm.save('./lstm/Quickdraw5Class1.h5')
    os.system(f'printf "model training finished" | mail -s "hello" chanssen@uw.edu')
except:
    os.system(f'printf "model training failed" | mail -s "{error}" chanssen@uw.edu')