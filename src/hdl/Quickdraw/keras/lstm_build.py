...
config_name = hls4ml.utils.config_from_keras_model(model, granularity='name', default_precision='ap_fixed<20,10>')
...
try:
    hls_model_name.build()
    os.system(f'printf "build finished" | mail -s "complete" chanssen@uw.edu')
except:
    os.system(f'printf "build failed" | mail -s "error" chanssen@uw.edu')