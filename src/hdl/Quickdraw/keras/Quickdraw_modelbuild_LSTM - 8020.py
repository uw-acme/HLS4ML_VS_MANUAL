...
# pad data to make sure it's valid
...
def makeRoc(features_val, labels_val, labels, model, outputDir='', outputSuffix=''):
    ...
    plt.savefig('%sROC_LSTM_8020%s.pdf'%(outputDir, outputSuffix))
    return labels_pred
...
def learningCurve(history):
    ...
    plt.savefig('LSTM_8020_learning_curve.pdf')
    plt.show()
    plt.close()
...
try:
    ...
    print(auc_score)
    lstm.save('./lstm/Quickdraw5Class1_8020.h5')
    os.system(f'printf "LSTM 8020 model training finished" | mail -s "hello" chanssen@uw.edu')
except:
    os.system(f'printf "LSTM 8020 model training failed" | mail -s "{error}" chanssen@uw.edu')