...
def makeRoc(features_val, labels_val, labels, model, outputDir='', outputSuffix=''):
    ...
    plt.savefig('%sROC_gru_20%s.pdf'%(outputDir, outputSuffix))
    return labels_pred
...
def learningCurve(history):
    ...
    plt.savefig('GRU_20_learning_curve.pdf')
    plt.show()
    plt.close()
...
try:
    ...
    history = gru.fit(array1_shuffled, array2_shuffled, batch_size = 256, epochs = 50, 
    ...
    print(auc_score)
    gru.save('./gru/Quickdraw5Class1_20_NEW.h5')
    os.system(f'printf "gru model new training finished" | mail -s "hello" chanssen@uw.edu')
except:
    os.system(f'printf "gru model new training failed" | mail -s "{error}" chanssen@uw.edu')