...
//	
//	// current slice of the image we're looking at and a copy for some reason?
...
//
//	// how many bits needed for the counter? (log base 2 of the number of pixels?)
//	//localparam BASE_LEVEL = ($clog2(inputWidth*inputWidth)) + 1;
//	// counter to keep track of how far we are into computing (why 8 bits??)
...
//	
//	// possibly delaying reading data for 1 clock cycle?
...
//		end else begin
//				if(counter == inputWidth && firstTime || firstTimeFullyThrough && 
//				   counter == (FIX_COUNT+1)+inputWidth**2 || counter == inputWidth **2-1) begin
...
//	// create in this case 2 sumNine modules which do the 'dot product' with the current 
//	// zeroedMatrix and corresponding weights
//	genvar i;
//	generate
...
//	// 		// if the current 'dot product' for each filter is negative, make it zero
...
//	// end
//	
//	// // 1 cycle delay from reluOutput[i] to tempSum[i]?? holds the same information
//	// always_ff @(posedge clk) begin
//	//    for( int i = 0; i < biasWidth; i++) begin
...
//	// end
//	
//	// // set oscillator to 0 on reset and then it flips every cycle
//	// always_ff @(posedge clk) begin
...
//
//
//	// this does not really make sense?? reluOutput has only 2 entries inside of it
//	// always_ff @(posedge clk) begin
